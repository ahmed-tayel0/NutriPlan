...
/* =========================================================================
   1. THEMEALDB RECIPE API
   Used for searching recipes, browsing by category/area, and reading a
   single recipe's full details.
   ========================================================================= */
...
// Get all recipes that belong to one category (e.g. "Seafood")
...
// Get all recipes that belong to one cuisine/area (e.g. "Italian")
...
// Get the list of all recipe categories (Beef, Chicken, Dessert, ...)
...
// Get the list of all cuisine areas (Italian, Mexican, Chinese, ...)
...
// TheMealDB stores each ingredient/measure as separate numbered fields
// (strIngredient1, strMeasure1, strIngredient2, strMeasure2, ...).
// This turns that into a clean array of { ingredient, measure } objects.
...
// TheMealDB stores instructions as one big block of text.
// This splits it into a clean array of individual steps.
function parseInstructionSteps(instructionsText) {
  ...
  for (const line of rawLines) {
    ...
    // remove a leading "1." or "1)" style number
    step = step.replace(/^\d+[.)]\s*/, "");

    // skip lines that are just "Step 1" or "1" with nothing else
...
// Group all the recipe functions together so they can be used like:
// mealDbApi.searchMealsByName("pasta")
const mealDbApi = {
  searchMealsByName,
  ...
  getAreaList,
  getMealById,
  extractIngredients: extractIngredientsList,
  parseInstructions: parseInstructionSteps,
};

/* =========================================================================
   2. NUTRITION ANALYSIS API
   Sends a recipe name + its ingredients to the NutriPlan nutrition API
   and gets back calories, protein, carbs, fat, etc.
   ========================================================================= */

const NUTRITION_API_BASE_URL = "https://nutriplan-api.vercel.app/api";
const NUTRITION_API_KEY = "qVdChEauj6UJ5NThudlFky059WQfTYp2EWT5BOHJ";

// Simple cache so we don't re-analyze the same recipe twice in one session
const recipeNutritionCache = new Map();

// Ask the nutrition API to analyze a recipe's ingredients
async function analyzeRecipeNutrition(recipeName, ingredientsList) {
  const cacheKey = `recipe_${recipeName}_${ingredientsList.join("|")}`;
  if (recipeNutritionCache.has(cacheKey)) {
    return recipeNutritionCache.get(cacheKey);
  }

  try {
    const response = await fetch(
      `${NUTRITION_API_BASE_URL}/nutrition/analyze`,
      {
        method: "POST",
        headers: {
    ...
    const formattedResult = {
      ...
      totalDaily: calculateDailyValuePercentages(nutritionData.totals),
    ...
    recipeNutritionCache.set(cacheKey, formattedResult);
...
// Turns raw nutrient totals into "% of a 2000-calorie daily diet" values
function calculateDailyValuePercentages(totals) {
  ...
  return {
    ...
    CHOCDF: {
      label: "Carbohydrates",
      quantity: Math.round((totals.carbs / recommendedDailyValues.carbs) * 100),
      unit: "%",
    },
    FIBTG: {
      label: "Fiber",
      quantity: Math.round((totals.fiber / recommendedDailyValues.fiber) * 100),
      unit: "%",
    },
    PROCNT: {
      label: "Protein",
...
// Used only if the real nutrition API call fails, so the app can still show something
...
// Turns the raw nutrition API response into simple numbers ready for the UI
function formatNutritionForDisplay(nutritionData) {
  ...
  // Newer API responses already give us "per serving" numbers directly
  ...
  // Older/fallback shape: divide the totals by the number of servings ourselves
...
const nutritionApi = {
  analyzeRecipe: analyzeRecipeNutrition,
  formatNutritionForDisplay,
};

/* =========================================================================
   3. OPEN FOOD FACTS PRODUCT API
   Used on the "Products" page to search packaged foods and look up
   products by barcode.
   ========================================================================= */

const OPEN_FOOD_FACTS_BASE_URL = "https://world.openfoodfacts.org";

// Search packaged food products by name / category / nutrition grade
async function searchProducts(searchOptions = {}) {
  try {
    ...
    const response = await fetch(
      `${OPEN_FOOD_FACTS_BASE_URL}/cgi/search.pl?${params}`,
    );
    ...
    return {
      ...
      products: (data.products || []).map(formatProductData),
    };
  } catch (error) {
    console.error("Error searching products:", error);
    return getFallbackProductData(searchOptions);
  }
}

// Look up one product by its barcode number
async function getProductByBarcode(barcode) {
  try {
    ...
    const data = await response.json();
    return data.status === 0 ? null : formatProductData(data.product);
  } catch (error) {
...
// Get products that belong to a category, e.g. "dairy"
async function getProductsByCategory(categoryId, page = 1, pageSize = 24) {
  try {
    const response = await fetch(
      `${OPEN_FOOD_FACTS_BASE_URL}/category/${encodeURIComponent(categoryId)}.json?page=${page}&page_size=${pageSize}`,
    );
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
// A fixed list of popular categories shown as quick-filter buttons
...
// Open Food Facts gives back messy raw data - this cleans it into a simple shape
function formatProductData(rawProduct) {
  return {
...
// Turns a nutrition grade letter (a-e) into a label/color/description
...
// Turns a NOVA processing group number (1-4) into a label/color/description
...
// Used only if the real product search fails, so the Products page still shows something
function getFallbackProductData(searchOptions = {}) {
  let sampleProducts = [
    {
      code: "7613034626844",
  ...
  if (searchOptions.nutritionGrade) {
    sampleProducts = sampleProducts.filter(
      (product) =>
        product.nutrition_grades === searchOptions.nutritionGrade.toLowerCase(),
    );
  }
  ...
  return {
    ...
    products: sampleProducts.map(formatProductData),
  };
}
...
/* =========================================================================
   4. APP STATE (in-memory data + localStorage persistence)
   ========================================================================= */

// Keys used to save/load data from the browser's localStorage
const STORAGE_KEYS = {
  DAILY_LOG: "nutriplan_daily_log",
  USER_SETTINGS: "nutriplan_user_settings",
};

// Settings used the first time the app runs (before the user changes anything)
...
// This object holds all of the app's live data while the page is open
let appData = {
  currentPage: "meals",
  searchQuery: "",
...
// Load saved settings + daily log from localStorage (or use defaults)
function initializeAppState() {
  const savedSettings = localStorage.getItem(STORAGE_KEYS.USER_SETTINGS);
  appData.userSettings = savedSettings
    ? JSON.parse(savedSettings)
    : { ...DEFAULT_USER_SETTINGS };

  const savedDailyLog = localStorage.getItem(STORAGE_KEYS.DAILY_LOG);
  appData.dailyLog = savedDailyLog ? JSON.parse(savedDailyLog) : {};
...
// Update the app state, and optionally save specific parts to localStorage
function updateAppState(changes, shouldPersist = false) {
  Object.assign(appData, changes);

  if (shouldPersist) {
    if (changes.dailyLog !== undefined) {
      localStorage.setItem(
        STORAGE_KEYS.DAILY_LOG,
        JSON.stringify(appData.dailyLog),
      );
    }
    if (changes.userSettings !== undefined) {
      localStorage.setItem(
        STORAGE_KEYS.USER_SETTINGS,
        JSON.stringify(appData.userSettings),
      );
    }
  }
...
// Merge new settings into the existing settings and save them
function updateUserSettings(newSettings) {
  appData.userSettings = { ...appData.userSettings, ...newSettings };
  updateAppState({ userSettings: appData.userSettings }, true);
}

// Today's date as "YYYY-MM-DD", used as the key for the daily log
...
/* =========================================================================
   5. UI TEMPLATE FUNCTIONS
   Small functions that return chunks of HTML (as strings) for cards,
   spinners, empty states, etc. Keeping them separate keeps the main
   app class easier to read.
   ========================================================================= */

// Card shown in the recipe grid
...
// Returns the color theme + icon used for a given recipe category
...
// Card shown in the category picker grid
...
// Simple spinning loader shown while data is being fetched
...
// Message shown when a search/filter returns no results
function createEmptyState(message, iconClass = "fa-search") {
  return `
...
// Row of "All Cuisines / Italian / Mexican / ..." filter buttons
function createAreaFilters(areasList, selectedArea = null) {
...
// Card shown in the product search results grid
...
// Full detail content shown inside the "product detail" popup
...
// Quick-filter button shown on the Products page ("Dairy", "Snacks", ...)
...
/* =========================================================================
   ...
   Wires everything above together: routing, page rendering, event
   listeners, and all the page-specific logic (Meals, Settings, Products,
   Food Log).
   ========================================================================= */

class NutriPlanApp {
  constructor() {
    this.state = appState.initializeAppState();
    this.currentPage = "meals";
    this.debounceTimer = null;
    this.loadStartTime = Date.now();
    this.minLoadDuration = 1000;

    // Maps a URL path to a page name
    this.routes = {
      "": "home",
      home: "meals",
      meals: "meals",
  ...
  async loadMealFromSlug(slug) {
    try {
      ...
      const matchingMeals = await mealDbApi.searchMealsByName(searchText);
      ...
      if (matchingMeals && matchingMeals.length > 0) {
        ...
        appState.updateAppState({ selectedMealId: meal.idMeal });
  ...
  // Turns a meal name into a URL-friendly slug, e.g. "Chicken Soup" -> "chicken-soup"
  slugify(text) {
    return text
      .toLowerCase()
      .trim()
  ...
  // Highlights the correct link in the sidebar navigation
  ...
  hideLoadingOverlay() {
    ...
    setTimeout(() => {
      // 1. ضيف كلاسات الـ Transition بـ Tailwind للتأكيد
      overlay.classList.add("transition-opacity", "duration-500", "ease-out");

      // 2. غير الـ opacity عشان يبدأ الـ Fade
      overlay.style.opacity = "0";

      // 3. استنى الـ 500ms بتوع الأنيميشن يخلصوا وبعدين اخفيه نهائياً
      setTimeout(() => {
  ...
  setupEventListeners() {
    ...
    // ✅ التقام الشامل لزرار Scan Product و Browse Recipes وأي اللينكات الداخلية
    document.addEventListener("click", (e) => {
      ...
      if (targetLink) {
        ...
        if (href) {
          // حالة 1: لو اللينك فيه هاش زي (#meals أو /foodlog#products)
          if (href.includes("#")) {
            e.preventDefault();
          ...
          // حالة 2: لو اللينك مسار مباشر زي (/products) زي زرار Scan Product
          else if (href.startsWith("/")) {
            e.preventDefault();
            const targetPage = href.replace("/", ""); // بيطلع كلمة "products"

            if (targetPage && this.routes[targetPage]) {
              // لو عايزه يظهر في الـ URL كـ /foodlog#products زي أخوه
  ...
  // Switches the recipe grid between a "grid" layout and a "list" layout
  setViewMode(mode) {
    const gridButton = document.getElementById("grid-view-btn");
    const listButton = document.getElementById("list-view-btn");
    ...
    appState.updateAppState({ viewMode: mode });
  }
  ...
  // One central click handler for all the dynamically-created buttons/cards
  ...
  // Debounces the search input so we don't call the API on every keystroke
  handleSearch(event) {
  ...
  async performSearch(query) {
    appState.updateAppState({ isLoading: true, searchQuery: query });

    const recipeGrid = document.querySelector("#all-recipes-section .grid");
    if (recipeGrid) recipeGrid.innerHTML = uiTemplates.createLoadingSpinner();

    try {
      const results = await mealDbApi.searchMealsByName(query);
      appState.updateAppState({ meals: results, isLoading: false });
      ...
      appState.updateAppState({ isLoading: false, error: error.message });
    }
  }
  ...
  async loadInitialData() {
    try {
      ...
      appState.updateAppState({ categories });

      const areas = await mealDbApi.getAreaList();
      appState.updateAppState({ areas });

      const defaultMeals = await mealDbApi.searchMealsByName("chicken");
      appState.updateAppState({ meals: defaultMeals });
    } catch (error) {
  ...
  async loadAllRecipes() {
    ...
    if (results.length === 0) {
      // The API needs a non-empty query, so fall back to a default search
      const defaultMeals = await mealDbApi.searchMealsByName("chicken");
      appState.updateAppState({ meals: defaultMeals });
      this.renderRecipeGrid(defaultMeals);
    } else {
      appState.updateAppState({ meals: results });
      this.renderRecipeGrid(results);
    }
  }
  ...
  renderPage(page) {
    ...
    // Hide every dynamically-created section before showing the right one
    [
      "settings-section",
      "products-section",
      "meal-detail-section",
  ...
  showMealsPage() {
    ...
    this.renderRecipeGrid(appState.getAppState().meals);
    this.renderAreaFilters();
  }

  // Shows or hides a list of section ids by their element id
  ...
  renderCategories() {
    ...
    const categories = appState.getAppState().categories || [];
    grid.innerHTML = categories
      .slice(0, 12)
      .map((category) => uiTemplates.createCategoryCard(category))
      .join("");
  }
  ...
  renderRecipeGrid(meals) {
    ...
    if (!meals || meals.length === 0) {
      grid.innerHTML = uiTemplates.createEmptyState(
    ...
    grid.innerHTML = meals
      .map((meal) => uiTemplates.createMealCard(meal))
      .join("");
  ...
  renderAreaFilters() {
    ...
    const areas = appState.getAppState().areas || [];
    const selectedArea = appState.getAppState().selectedArea;
    filterContainer.innerHTML = uiTemplates.createAreaFilters(
      areas.slice(0, 10),
      selectedArea,
    );
  }
  ...
  async filterByCategory(category) {
    appState.updateAppState({ selectedCategory: category, isLoading: true });

    const grid = document.querySelector("#all-recipes-section .grid");
    if (grid) grid.innerHTML = uiTemplates.createLoadingSpinner();

    try {
      const summaryList = await mealDbApi.filterMealsByCategory(category);
      const fullMeals = await Promise.all(
        summaryList
          .slice(0, 20)
          .map((meal) => mealDbApi.getMealById(meal.idMeal)),
      );
      const validMeals = fullMeals.filter((meal) => meal);

      appState.updateAppState({ meals: validMeals, isLoading: false });
      ...
      appState.updateAppState({ isLoading: false });
    }
  }

  async filterByArea(area) {
    appState.updateAppState({ selectedArea: area, isLoading: true });
    ...
    try {
      let meals;
      if (area) {
        const summaryList = await mealDbApi.filterMealsByArea(area);
        const fullMeals = await Promise.all(
          summaryList
            .slice(0, 20)
            .map((meal) => mealDbApi.getMealById(meal.idMeal)),
      ...
      appState.updateAppState({ meals, isLoading: false });
  ...
  async showMealDetail(mealId) {
    appState.updateAppState({ selectedMealId: mealId, isLoading: true });
  ...
  async showMealDetailPage() {
    ...
    const mealId = appState.getAppState().selectedMealId;
    if (!mealId) {
      detailSection.innerHTML = `
                <div class="max-w-6xl mx-auto">
                    ...
                    ${uiTemplates.createEmptyState("No recipe selected. Please select a recipe to view details.", "fa-utensils")}
                </div>
            `;
      document
    ...
    try {
      ...
      appState.updateAppState({ selectedMeal: meal, isLoading: false });
      ...
      detailSection.innerHTML = `
                <div class="max-w-6xl mx-auto">
                    ...
                    ${uiTemplates.createEmptyState("Failed to load recipe details. Please try again.", "fa-exclamation-circle")}
                </div>
            `;
      document
        .getElementById("back-to-meals-btn")
  ...
  // Fetches the nutrition facts for a recipe and fills in the nutrition panel
  async loadNutritionData(meal, ingredients) {
    ...
    try {
      ...
      const rawNutritionData = await nutritionApi.analyzeRecipe(
        meal.strMeal,
        ingredientLines,
      );
      const formattedNutrition =
        nutritionApi.formatNutritionForDisplay(rawNutritionData);

      const nutritionCache = appState.getAppState().mealNutritionCache || {};
      nutritionCache[meal.idMeal] = formattedNutrition;
      appState.updateAppState({ mealNutritionCache: nutritionCache });
  ...
  // Builds the "Nutrition Facts" panel HTML from formatted nutrition data
  ...
  // Builds the full recipe detail page HTML
  ...
  closeMealDetail() {
    this.navigateTo("meals");
    appState.updateAppState({ selectedMeal: null, selectedMealId: null });
  }

  // Small toast notification shown in the bottom-right corner
  ...
  renderSettingsSection() {
    ...
    const settings = appState.getAppState().userSettings;
  ...
  async renderProductsSection() {
    ...
    const popularCategories = await foodProductApi.getPopularCategories();
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex gap-3 overflow-x-auto pb-2">
                        ${popularCategories.map((category) => uiTemplates.createProductCategoryButton(category)).join("")}
                    </div>
                </div>
                ...
                <div id="products-loading" class="hidden py-12">
                    ${uiTemplates.createLoadingSpinner()}
                </div>
  ...
  async searchProducts(query, nutritionGrade = "") {
    ...
    try {
      ...
      const results = await foodProductApi.searchProducts(searchOptions);
      loadingIndicator.classList.add("hidden");

      if (results.products.length > 0) {
        grid.innerHTML = results.products
          .map((product) => uiTemplates.createProductCard(product))
          .join("");
      ...
      appState.updateAppState({ searchedProducts: results.products });
    } catch (error) {
  ...
  async searchProductsByCategory(categoryId) {
    ...
    try {
      const results = await foodProductApi.getProductsByCategory(categoryId);
      loadingIndicator.classList.add("hidden");
  ...
  async lookupBarcode(barcode) {
    ...
    try {
      const product = await foodProductApi.getProductByBarcode(barcode);
      loadingIndicator.classList.add("hidden");

      if (product) {
        grid.innerHTML = uiTemplates.createProductCard(product);
        countLabel.textContent = `Found product: ${product.name}`;
        appState.updateAppState({ searchedProducts: [product] });
  ...
  async showProductDetail(barcode) {
    let product = appState
      .getAppState()
      .searchedProducts?.find((item) => item.barcode === barcode);
    if (!product) {
      product = await foodProductApi.getProductByBarcode(barcode);
    }
    if (!product) {
    ...
    const nutriScoreInfo = foodProductApi.getNutriScoreInfo(
      product.nutritionGrade,
    );
    const novaGroupInfo = foodProductApi.getNovaGroupInfo(product.novaGroup);
    ...
    modal.innerHTML = `
            <div class="bg-white rounded-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                ${uiTemplates.createProductDetailContent(product, nutriScoreInfo, novaGroupInfo)}
            </div>
        `;
  ...
  // --- Food logging (shared by Products page and Meal detail page) -----

  logFoodToDaily(product) {
    const today = appState.getTodayDateString();
    const dailyLog = appState.getAppState().dailyLog || {};
    ...
    appState.updateAppState({ dailyLog }, true);
    this.showNotification(
  ...
  showLogMealModal(meal) {
    const nutrition = appState.getAppState().mealNutritionCache?.[meal.idMeal];
    ...
    modal.querySelector("#confirm-log-meal")?.addEventListener("click", () => {
      ...
      const nutritionForLog =
        appState.getAppState().mealNutritionCache?.[meal.idMeal] || nutrition;
  ...
  renderFoodLogSection() {
    ...
    // Use the user's real nutrition goals from Settings (with sensible fallbacks)
    const settings =
      appState.getAppState().userSettings || DEFAULT_USER_SETTINGS;
    const goals = {
  ...
  // Builds one progress bar (used four times: Calories/Protein/Carbs/Fat)
  ...
  // Builds the list of today's logged meals/products
  ...
  // Builds an array of the last 7 days with each day's total calories + item count
  getWeeklyLogData() {
    const dailyLog = appState.getAppState().dailyLog || {};
  ...
  // Re-renders the Food Log page only if it's the page currently on screen
  updateFoodLogPage() {
  ...
  getTodayLogSummary() {
    const today = appState.getTodayDateString();
    return (
      (appState.getAppState().dailyLog || {})[today] || {
        totalCalories: 0,
        totalProtein: 0,
        totalCarbs: 0,
        totalFat: 0,
